...
def _calibrar_campo_from_pts_gps(pts_gps: dict, epsg: int):
    """
    Recebe cantos em lat/lon e devolve os mesmos outputs do _calibrar_campo:
    pts_gps, (clat, clon), pts_utm, origin, R, angulo_rad, dist_x, dist_y
    ...
    dist_x = float(np.linalg.norm(pts_utm["BR"] - pts_utm["BL"]))
    dist_y = float(np.linalg.norm(pts_utm["TL"] - pts_utm["BL"]))
    ...
    return pts_gps, (clat, clon), pts_utm, origin, R, angulo_rad, dist_x, dist_y
