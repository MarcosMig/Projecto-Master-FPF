...
# Persistência de inputs entre steps (file_uploader perde ficheiros se não renderizado)
if "ss_f_atleta" not in st.session_state:
    st.session_state.ss_f_atleta = []
if "ss_f_campo" not in st.session_state:
    st.session_state.ss_f_campo = []
if "ss_metodo_campo" not in st.session_state:
    st.session_state.ss_metodo_campo = "Upload dos 4 cantos"
if "ss_data_sessao" not in st.session_state:
    st.session_state.ss_data_sessao = pd.Timestamp.today().date()
if "ss_selecao" not in st.session_state:
    st.session_state.ss_selecao = SELECOES_OPCOES[0]
if "ss_contexto" not in st.session_state:
    st.session_state.ss_contexto = "Treino"
if "ss_adversario" not in st.session_state:
    st.session_state.ss_adversario = ""
...
def _render_step_indicator():
    """Render horizontal step indicator bar at the top."""
    current = st.session_state.wizard_step
    cols = st.columns(len(WIZARD_STEPS))
    for i, (step_num, label) in enumerate(WIZARD_STEPS.items()):
        with cols[i]:
            if step_num == current:
                st.markdown(
                    f"<div style='text-align:center; padding:8px 0; background:#1e6fff; border-radius:20px; "
                    f"color:white; font-weight:700; font-size:0.95rem;'>"
                    f"{step_num}. {label}</div>",
                    unsafe_allow_html=True,
                )
            elif step_num < current:
                st.markdown(
                    f"<div style='text-align:center; padding:8px 0; background:#1a1c23; border-radius:20px; "
                    f"color:#4caf50; font-weight:600; font-size:0.95rem; border:1px solid #30363d;'>"
                    f"✓ {label}</div>",
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f"<div style='text-align:center; padding:8px 0; background:#1a1c23; border-radius:20px; "
                    f"color:#6b7280; font-weight:500; font-size:0.95rem; border:1px solid #30363d;'>"
                    f"{step_num}. {label}</div>",
                    unsafe_allow_html=True,
                )
...
def _format_session_header_summary():
    data_sessao = st.session_state.get("ss_data_sessao")
    selecao = st.session_state.get("ss_selecao", SELECOES_OPCOES[0])
    contexto = st.session_state.get("ss_contexto", "Treino")
    adversario = st.session_state.get("ss_adversario", "")

    if hasattr(data_sessao, "strftime"):
        data_label = data_sessao.strftime("%d/%m/%Y")
    else:
        data_label = str(data_sessao)

    resumo = [data_label, selecao, contexto]
    if contexto == "Jogo" and adversario:
        resumo.append(f"vs {adversario}")

    return " | ".join([item for item in resumo if item])


# --- INTERFACE WIZARD ---
_header_left, _header_right = st.columns([5, 2])
with _header_left:
    st.title("Validação de Dados")
    st.markdown(
        f"<div style='text-align:left; font-size:18px; color:#9aa0a6; margin-bottom:0.5rem;'>{_format_session_header_summary()}</div>",
        unsafe_allow_html=True,
    )
with _header_right:
    st.markdown(
        f"<div style='text-align:right; font-size:16px; color:#9aa0a6; margin-top:0.8rem;'>User: {st.session_state.login_user} | FPF</div>",
        unsafe_allow_html=True,
    )
...
if st.session_state.wizard_step == 1:
    if "ui_data_sessao" not in st.session_state:
        st.session_state.ui_data_sessao = st.session_state.ss_data_sessao
    if "ui_selecao" not in st.session_state:
        st.session_state.ui_selecao = st.session_state.ss_selecao
    if "ui_contexto" not in st.session_state:
        st.session_state.ui_contexto = st.session_state.ss_contexto
    if "ui_adversario" not in st.session_state:
        st.session_state.ui_adversario = st.session_state.ss_adversario

    with st.container(border=True):
        st.header("Dados da Sessão")
        estadio = None
        data_sessao = st.date_input("Data do Evento", key="ui_data_sessao")
        selecao = st.selectbox(
            "Seleção",
            options=SELECOES_OPCOES,
            index=SELECOES_OPCOES.index(st.session_state.ui_selecao),
            key="ui_selecao",
        )
        ...
        contexto = st.selectbox(
            "Contexto",
            options=["Treino", "Jogo"],
            index=["Treino", "Jogo"].index(st.session_state.ui_contexto),
            key="ui_contexto",
        )

        adversario = ""
        if contexto == "Jogo":
            adversario = st.text_input("Adversário", key="ui_adversario")

    # Navigation
    _nav_c1, _nav_c2 = st.columns([4, 1])
    with _nav_c2:
        if st.button("Continuar para Atletas ➜", type="primary", use_container_width=True):
            st.session_state.ss_data_sessao = st.session_state.ui_data_sessao
            st.session_state.ss_selecao = st.session_state.ui_selecao
            st.session_state.ss_contexto = st.session_state.ui_contexto
            st.session_state.ss_adversario = (
                st.session_state.get("ui_adversario", "").strip()
                if st.session_state.ui_contexto == "Jogo"
                else ""
            )
            st.session_state.ss_genero = genero
            _go_to_step(2)
    st.stop()
...
if st.session_state.wizard_step == 2:
    with st.container(border=True):
        ...
        f_atleta_upload = st.file_uploader(
            "Arrasta os CSVs dos atletas", accept_multiple_files=True, type=["csv"],
            help="Formato .csv · até 200 MB"
        )
        if f_atleta_upload:
            st.session_state.ss_f_atleta = f_atleta_upload
    ...
    with _nav_c2:
        if st.button("⬅ Voltar", use_container_width=True):
            _go_to_step(1)
    with _nav_c3:
        if st.button("Continuar para Campo ➜", type="primary", use_container_width=True):
            if not st.session_state.ss_f_atleta:
                st.error("Carrega pelo menos um ficheiro de atleta.")
            else:
                _go_to_step(3)
    st.stop()
...
if st.session_state.wizard_step == 3:
    with st.container(border=True):
        st.header("Calibração do Campo")
        st.caption(
            "Defina os 4 cantos para calibrar o casetas."
        )
        ...
        metodo_campo = st.radio(
            ...
            index=["Upload dos 4 cantos", "Selecionar no mapa", "Usar campo guardado"].index(
                st.session_state.ss_metodo_campo
            ) if st.session_state.ss_metodo_campo in ["Upload dos 4 cantos", "Selecionar no mapa", "Usar campo guardado"] else 0,
            captions=[
                "Carrega um CSV com as coordenadas BL/BR/TL/TR",
                "Escolhe os 4 cantos diretamente no mapa interativo",
                "Seleciona um campo guardado anteriormente",
            ],
        )
        st.session_state.ss_metodo_campo = metodo_campo

        if metodo_campo == "Upload dos 4 cantos":
            f_campo_upload = st.file_uploader(
                "Dados de CAMPO (BL, BR, TL, TR)", accept_multiple_files=True, type=["csv"]
            )
            if f_campo_upload:
                st.session_state.ss_f_campo = f_campo_upload

    # Map picking logic — reset on method change
    if (
        metodo_campo == "Selecionar no mapa"
    ...
    _campo_ok = False
    if metodo_campo == "Upload dos 4 cantos" and st.session_state.ss_f_campo:
        _campo_ok = True
    elif metodo_campo == "Selecionar no mapa" and st.session_state.get("pts_gps_picked") is not None:
        _campo_ok = True
    elif metodo_campo == "Usar campo guardado":
        _campo_ok = True  # validated later in step 4

    # --- Map picking UI is rendered below (after function defs) ---
...
# ======================== STEP 3 (continued): MAP PICKER ========================
if st.session_state.wizard_step == 3:
    metodo_campo = st.session_state.ss_metodo_campo
    f_atleta = st.session_state.ss_f_atleta

    if metodo_campo == "Selecionar no mapa" and st.session_state.get("pts_gps_picked") is None:
        ...
        st.subheader("Pick dos 4 cantos do campo")
        st.caption(
    ...
    with _nav_c2:
        if st.button("⬅ Voltar", use_container_width=True, key="step3_back"):
            _go_to_step(2)
    with _nav_c3:
        if st.button("Continuar para Validação ➜", type="primary", use_container_width=True, key="step3_next"):
            if not _campo_ok:
                st.error("Define os cantos do campo antes de continuar.")
            else:
                _go_to_step(4)
    st.stop()

# ======================== STEP 4: VALIDAR + PROCESSAR ========================
if st.session_state.wizard_step == 4:
    # Recover variables from session_state
    f_atleta = st.session_state.ss_f_atleta
    f_campo = st.session_state.ss_f_campo
    metodo_campo = st.session_state.ss_metodo_campo
    data_sessao = st.session_state.get("ss_data_sessao", None)
    selecao = st.session_state.get("ss_selecao", SELECOES_OPCOES[0])
    genero = selecao.split()[-1] if selecao.split() and selecao.split()[-1] in ["M", "F"] else ""
    contexto = st.session_state.get("ss_contexto", "Treino")
    adversario = st.session_state.get("ss_adversario", "")
    ...
    # Guard: if session data is missing (e.g. page refresh), go back to step 1
    if not f_atleta or data_sessao is None:
        st.warning("Dados de sessão em falta. A voltar ao primeiro passo...")
        _go_to_step(1)

    # Summary header
    _ctx_label = "Sessão de treino" if contexto == "Treino" else f"Jogo vs {adversario}" if adversario else "Jogo"
    _date_label = data_sessao.strftime("%d %b %Y") if hasattr(data_sessao, "strftime") else str(data_sessao)

    _hero_left, _hero_right = st.columns([4.5, 1.2])
    with _hero_left:
        st.markdown("## Pronto para validar e processar")
        st.caption("Revê a sessão, a calibração do campo e os ficheiros de atletas antes de gerar o relatório.")
    with _hero_right:
        if st.button("⬅ Voltar ao Campo", key="step4_back", use_container_width=True):
            _go_to_step(3)
    ...
    atletas_total = len(audit_data)
    ficheiros_total = len(f_atleta)

    with st.container(border=True):
        st.markdown("### Resumo final da sessão")
        resumo_top = st.columns(4)
        with resumo_top[0]:
            st.markdown(f"**Data**  \n{_date_label}")
        with resumo_top[1]:
            st.markdown(f"**Seleção**  \n{selecao}")
        with resumo_top[2]:
            st.markdown(f"**Contexto**  \n{contexto}")
        with resumo_top[3]:
            st.markdown(f"**Campo**  \n{metodo_campo}")

        resumo_metrics = st.columns(3)
        with resumo_metrics[0]:
            st.metric("Ficheiros de atletas", ficheiros_total)
        with resumo_metrics[1]:
            st.metric("Atletas detetados", atletas_total)
        with resumo_metrics[2]:
            st.metric("Atletas completos", f"{completos}/{atletas_total}" if atletas_total else "0/0")

        if contexto == "Jogo":
            st.caption(f"Adversário: {adversario or '—'}")

    try:
        if metodo_campo == "Selecionar no mapa":
    ...
    campo_status = "Campo calibrado" if pts_gps else "Campo por validar"
    geo_status = "Validação pronta" if passed_geo else "Validação bloqueada"

    with st.container(border=True):
        info_cols = st.columns(4)
        with info_cols[0]:
            st.markdown(f"**Campo, Local**  \n{campo_local}")
        with info_cols[1]:
            st.markdown(f"**Atletas, Local**  \n{atletas_local}")
        with info_cols[2]:
            st.metric("Atletas OK", f"{pct_ok*100:.0f}%")
        with info_cols[3]:
            st.markdown(f"**Estado do campo**  \n{campo_status}")

        if passed_geo:
            st.success(f"{geo_status}. A sessão pode avançar para processamento com {pct_ok*100:.0f}% dos atletas dentro do raio definido.")
        else:
            st.warning(f"{geo_status}. Revê o campo ou os ficheiros antes de processar. Atletas OK: {pct_ok*100:.0f}%.")
    ...
    st.subheader("Auditoria de Atletas")
    ...
    with st.container(border=True):
        actions_head_left, actions_head_right = st.columns([3.4, 1.1])
        with actions_head_left:
            st.markdown("### Validar e gerar relatório")
            st.caption("A ação principal executa a normalização, sincronização, cálculo de métricas e geração do relatório final.")
        with actions_head_right:
            st.caption("Ação opcional")
            with st.popover('💾 Guardar Campo', use_container_width=True):
        ...
        btn_row = st.columns([1.35, 1.9])
        with btn_row[0]:
            btn = st.button("⚙️ Validar e Gerar Relatório", type="primary", use_container_width=True)
        with btn_row[1]:
            st.caption("Confirma a sessão final antes de iniciar o processamento.")
        ...
        with st.status("A iniciar processamento...", expanded=True) as status:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                session_fingerprint = hash_session(
                    data_sessao, selecao, genero, contexto, estadio, adversario, f_campo, f_atleta
                )
