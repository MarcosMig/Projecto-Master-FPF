...
def audit_timebase(df: pd.DataFrame, col_time: str, expected_hz: float = 10.0) -> dict:
    ...
    return {
        ...
        "n_dt_neg": n_dt_neg,
        "n_dt_zero": n_dt_zero,
        "n_gaps_gt_0_2s": int((dt_pos > 0.2).sum()),
        "n_gaps_gt_2s": int((dt_pos > 2.0).sum()),
    }
...
def compute_metrics_for_df(df: pd.DataFrame) -> dict:
    ...
    good = dt > 0
    out["n_gaps_gt2s"] = int(np.sum(dt[good] > 2.0)) if np.any(good) else 0
    ...
    active_time_s = float(np.nansum(dt[v >= 0.5])) if len(v) else 0.0
    ...
    if len(t_cum) == len(dist_cum) and len(t_cum) > 1:
        ...
        for i in range(len(t_cum)):
            while j < len(t_cum) and t_cum[j] - t_cum[i] <= 60.0:
                j += 1
            if j - 1 >= i:
                best = max(best, dist_cum[j - 1] - dist_cum[i])
        peak_1m = float(best)
    ...
    out.update(
        {
            ...
            "n_sprints": count_bouts(t[1:][good], v >= SPRINT_MPS, SPRINT_BOUT_MIN_S),
            "n_acc_2_5": int(np.nansum(acc >= ACC_THR)),
            "n_dec_3_0": int(np.nansum(acc <= DEC_THR)),
            "vmax_mps": vmax,
